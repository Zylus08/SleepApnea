...
def evaluate_mil():
    ...
    # 1. Load Labels and Recreate Test Split (Seed 42)
    ...
    # 2. Load the AB-MIL Model
    base_encoder = EEGEncoder(in_channels=20)
    model = AttentionMIL(base_encoder).to(device)
    model.load_state_dict(torch.load('E:/SleepApnea/SleepApneaSSL/mil_finetuned_model.pth', weights_only=True))
    model.eval()
    ...
    print(f"\n==========================================")
    print(f" FINAL AB-MIL PATIENT-LEVEL AUC: {roc_auc:.4f}")
    print(f"==========================================")
